...
To check how the model transfers beyond CARLA simulation, we applied it to a real-world front-facing dashcam clip (768×432, 30fps, subsampled to 8fps / 64 frames).

<figure>
  <img src="/images/viz_dashcam_chunk4.gif" alt="AutoGaze on real-world dashcam footage">
  <figcaption>AutoGaze (gazing_ratio=0.75, chunk_size=4) applied to real dashcam footage. The model selects ~54% of patches — higher than on CARLA data at the same ratio, likely reflecting the greater visual complexity of real road scenes.</figcaption>
</figure>

Coverage at gazing_ratio=0.75 is ~54% on the dashcam clip vs ~53% on CARLA — essentially the same. The patch distribution looks visually sensible: road surface, vehicles, and signage receive the most attention.
